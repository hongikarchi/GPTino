...
## 네이티브 UX 스펙 (사용자 확정, 2026-08-05 저녁 — "터미널 편안함 + 뷰포트 직관성")

**작동 개념**: 가능한 한 결정론적 코드로 작동하되, 애매함·사람 판단 필요 지점·edge case
+ 프로젝트별 커스텀 C# 작성을 LLM이 메꾼다 (예: SelDup은 정확 일치만 — 부분 겹침/포함
같은 애매한 중복 판단은 LLM이). Rhino를 잘 못 다루는 사람도 채팅으로 기능을 구현/사용.

**요구 UX**: ① GPTino의 역질문이 채팅에 오면 그 질문을 클릭 → 뷰포트에서 해당 부위
줌인/hide isolate/lock isolate로 확인 ② 솔루션 알트(alt 1/2/3 + 텍스트 설명)도 클릭 →
뷰포트에서 직관적으로 형태 확인 ③ GPTino가 스스로 줌인·아이솔레이트하며 "여기를
말하는 겁니다"라고 보여주는 에이전트 주도 포커스.

**구현 자산·경로** (조사 확정): `POST /api/v1/focus` 엔드포인트가 이미 존재
(Program.cs:328, FocusObjectsCoreAsync — select/isolate/lock/restore + Zoom, 복원 스택;
curator 감사카드용, 의도적으로 에이전트 툴 제외). 남은 빌드: ⓐ 채팅 메시지에
"포커스 참조"(objectIds+mode) 첨부 스키마 + 패널에서 클릭 칩 렌더 → /focus 호출
(curator 카드 컴포넌트 재사용) ⓑ 에이전트 주도 포커스 = /focus의 에이전트 노출 여부
설계 결정(복원 예절 필수 — curator 트랙과 조정) ⓒ 질문/알트 → 실제 객체 GUID 매핑은
파이프라인이 산출(focus-targets.json 실증됨).

**스펙 3 답변 기록**: 구조 모델의 자유단들 = 의도된 캔틸레버(확정), 지붕 가새 자유단
= 모델링 실수 가능성(플래그 유지).

## 실무 수용 기준 (사용자 스펙, 2026-08-05 — 테스트 파일 "260803 main ms.3dm" 통과가 합격)

1. **추출**: 기성 구조 부재(블록 정리형 + 날 것의 mesh/brep형 둘 다)에서 구조 선·단면을
   **정확히** 추출해 GH 입력으로. 선이 겹치거나 애매하게 틀어지면 불합격 — 건물 상식
   (직교 그리드 + 의도적 사선만) 기준의 품질 감사 필수.
2. **접합 검증·수선**: 지점/노드가 제대로 만나는지 확인, 이상 시 자동 수정
   (부재가 안 만나 뜬금없는 캔틸레버가 되는 일 금지).
3. **해석 + 대화형 확인**: 애매한 것(지점 위치, 캔틸레버 의도, 가새 여부)은 GPTino가
   역으로 채팅으로 질문.
4. **PyNite 해석 → Rhino 시각화**: 수치보다 "어디가 문제인지 직관적으로 보이는 것"이 핵심.
5. **솔루션 제안**: 문제 부위에 단면 확대/부재 추가/스팬 축소 대안을 채팅으로 제시,
   알트 선택 시 시각적으로 비교.
