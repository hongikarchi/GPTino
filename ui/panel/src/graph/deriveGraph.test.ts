...
describe("deriveGraph", () => {
  ...
  it("summarizes dataFlow as resting doc chips, never as canvas arcs", () => {
    const state = createDemoRuntimeState();

    // The ledger itself lives on the Data tab; the Model canvas keeps only the passive chip.
    ...
    const legacyModel = deriveGraph(legacy);
    expect(node(legacyModel, "doc:gh").dataChip).toBe("⇢12 ⇠38");

    const ambiguous = createDemoRuntimeState();
    ambiguous.grasshopperDocs = null;
    expect(node(deriveGraph(ambiguous), "doc:gh").dataChip).toBeUndefined();
  });
