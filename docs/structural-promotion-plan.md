...
| 항목 | 판정 Phase |
|---|---|
| py39-rh8 python.exe 경로가 AgentHost 프로세스에서 안정 탐색되는가 | 2 |
| 프로세스 밖 PyNite 1,199부재 해석 시간 (0.85초가 스폰 오버헤드 포함에도 유지되는가) | 2 |
| 판정색 bake의 typed op 조합이 부재 수십 개 수준에서 1 ChangeSet에 담기는가 | 3 |
| 자유단 역질문이 hygiene 게이트처럼 goal 카드를 먼저 띄우는가 (흐름 순서 실측) | 3 |
