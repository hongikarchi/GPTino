...
### Phase 5 — 종합 라이브 게이트 + 실무 검증 (세션 1회)

`gate-structural.ps1` (gate-approval.ps1 패턴): 픽스처에서 추출→역질문→해석→시각화→대안→적용
전 경로, 관측점 2개 이상(아티팩트 수치 + `/dev/audit`류 독립 프로브) 교차. 마지막으로 실무
3dm을 사용자 입회 하에 통과 — **수용 기준 5항이 이 파일 기준이므로 이게 최종 합격선**.
...
| 항목 | 판정 Phase |
|---|---|
| ~~py39-rh8 python.exe 경로가 AgentHost 프로세스에서 안정 탐색되는가~~ | ✅ 게이트에서 확인 (solve 1.17s) |
| 프로세스 밖 PyNite **1,199부재** 해석 시간 (실무 3dm에서) | 실무 검증 |
| 판정색 bake의 typed op 조합이 부재 수십 개 수준에서 1 ChangeSet에 담기는가 | 실패 부재가 실제로 나오는 씬에서 (게이트는 10/10 통과라 미발화) |
| goal 카드가 구조 흐름에서 언제 발화하는가 (이번 게이트는 역질문 프로즈로 충분했음) | 실무 검증 |
